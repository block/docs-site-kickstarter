...
```tsx
<BlogSection layout="grid" columns={3} posts={posts} />
```
...
Adjust grid layout for different content densities:

```tsx
<!-- Dense layout for many posts -->
<BlogSection layout="grid" columns={4} posts={posts} />

<!-- Spacious layout for detailed posts -->
<BlogSection layout="grid" columns={2} posts={posts} />
```
...
- **Desktop**: Full grid/list layout with all metadata
